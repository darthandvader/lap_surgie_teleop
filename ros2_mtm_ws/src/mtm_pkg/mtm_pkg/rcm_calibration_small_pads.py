...
class ArucoPosePublisher(Node):
    def __init__(self):
        ...
        # TODO: adjust the transformation: from left/right markers to left/right RCMs.
        self.marker_T_right_rcm = np.eye(4)
        # self.marker_T_right_rcm[:3, 3] = np.array([0.085, 0.065, 0.0])
        self.marker_T_right_rcm[:3, 3] = np.array([0.0, 0.0, 0.0])
        self.marker_T_left_rcm = np.eye(4)
        # self.marker_T_left_rcm[:3, 3] = np.array([-0.085, 0.065, 0.012])
        self.marker_T_left_rcm[:3, 3] = np.array([0.0, 0.0, 0.0])
        ...
        cv2.resizeWindow("k", 1, 1)   # tiny window just to capture keys
    ...
    def _draw_rcm_axes_on_image(self, img, rcm_R_cam, rcm_t_cam, axis_len=0.05, label='RCM'):
        ...
        rcm_R_cam: 3x3 rotation (RCM->camera)
        rcm_t_cam: (3,) translation (RCM->camera) in meters
        """
        # points in the RCM frame
        obj_pts = np.float32([
            [0, 0, 0],                  # origin
            [axis_len, 0, 0],           # X
            [0, axis_len, 0],           # Y
        ...
        # Convert to rvec/tvec for OpenCV projection
        ...
        O, X, Y, Z = img_pts
        # axes colors: X=red, Y=green, Z=blue (OpenCV BGR)
        cv2.circle(img, O, 3, (255, 255, 255), -1)
        cv2.line(img, O, X, (0, 0, 255), 2)
        cv2.line(img, O, Y, (0, 255, 0), 2)
        cv2.line(img, O, Z, (255, 0, 0), 2)
        cv2.putText(img, label, (O[0] + 5, O[1] - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (220, 220, 220), 1, cv2.LINE_AA)
    ...
    def detection_callback(self):
        ...
        # If frozen: just re-publish last known poses and skip detection
        if self.freeze:
            if hasattr(self, "last_rcm_left") and self.last_rcm_left is not None:
                self.last_rcm_left.header.stamp = getattr(self, 'time_stamp', self.get_clock().now().to_msg())
                self.rcm_left_pub.publish(self.last_rcm_left)
            if hasattr(self, "last_rcm_right") and self.last_rcm_right is not None:
                self.last_rcm_right.header.stamp = getattr(self, 'time_stamp', self.get_clock().now().to_msg())
        ...
        for idx, marker_id in enumerate(flat_ids):
            ...
            # --- Compose RCM in camera frame and (optionally) draw ---
        ...
        # --- Publish overlay only if enabled ---
        if self.show_overlay:
            try:
                overlay_msg = self.bridge.cv2_to_imgmsg(overlay, encoding='bgr8')
                overlay_msg.header = self.last_image_msg.header
