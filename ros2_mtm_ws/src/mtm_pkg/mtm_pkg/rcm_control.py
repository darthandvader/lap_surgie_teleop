...
class SE3Smoother:
    """
    First-order low-pass on position and orientation.
    Works with pin.SE3 or 4x4 numpy; returns same type as input.
    tau_* are time constants in seconds (smaller = less smoothing).
    """
    def __init__(self, tau_pos=0.03, tau_rot=0.03):
        self.tau_pos = float(tau_pos)
        self.tau_rot = float(tau_rot)
        self._p = None            # last commanded position (3,)
        self._q = None            # last commanded quaternion (xyzw)

    @staticmethod
    def _alpha(dt, tau):
        return 1.0 if tau <= 0.0 else dt / (tau + dt)

    def step(self, T_new, dt):
        # accept pin.SE3 or 4x4
        if hasattr(T_new, "homogeneous"):
            Tm = T_new.homogeneous
            return_as_se3 = True
        else:
            Tm = np.asarray(T_new)
            return_as_se3 = False
        ...
        p_new = Tm[:3, 3]
        q_new = R.from_matrix(Tm[:3, :3]).as_quat().astype(float)  # xyzw
        ...
        # --- position LPF ---
        a_pos = self._alpha(dt, self.tau_pos)
        self._p = (1.0 - a_pos) * self._p + a_pos * p_new

        # --- orientation LPF (on SO(3)) ---
        # ensure shortest path
        if np.dot(self._q, q_new) < 0.0:
            q_new = -q_new
        Rc = R.from_quat(self._q)
        Rt = R.from_quat(q_new)
        dR = Rc.inv() * Rt                    # current -> target
        a_rot = self._alpha(dt, self.tau_rot)
        dq = R.from_rotvec(dR.as_rotvec() * a_rot)
        self._q = (Rc * dq).as_quat()

        # reassemble
        T = np.eye(4)
        T[:3, :3] = R.from_quat(self._q).as_matrix()
        T[:3, 3] = self._p
        if return_as_se3:
