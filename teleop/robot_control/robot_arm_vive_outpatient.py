...
def data_handler_worker(ip, LR, name, network, left_buf, right_buf):
    handler = inspire_sdk.ModbusDataHandler(
        network=network, ip=ip, LR=LR, device_id=1, initDDS=False
    )

    sim_order = [3, 2, 1, 0, 5, 4]
    time.sleep(0.5)
    try:
        while True:
            data_dict = handler.read()
            if name == "Right Hand Process":
                right_buf.value = sensor_data_to_image(data_dict, "right")
                right_hand_joint_position[:] = data_dict["states"]["ANGLE_ACT"]
                right_pose = np.array(right_hand_joint_position, dtype=np.float32)

                right_pose = right_pose[sim_order]
                for i in range(6):
                    right_pose[i] = map_value(
                        right_pose[i],
                        1000,
                        0,
                        right_joint_limits[str(i)][0],
                        right_joint_limits[str(i)][1],
                    )
                right_hand_joint_position[:] = right_pose.tolist()
            else:
                left_buf.value = sensor_data_to_image(data_dict, "left")
                left_hand_joint_position[:] = data_dict["states"]["ANGLE_ACT"]
                left_pose = np.array(left_hand_joint_position, dtype=np.float32)
                left_pose = left_pose[sim_order]
                for i in range(6):
                    left_pose[i] = map_value(
                        left_pose[i],
                        1000,
                        0,
                        left_joint_limits[str(i)][0],
                        left_joint_limits[str(i)][1],
                    )
                left_hand_joint_position[:] = left_pose.tolist()
...
if __name__ == "__main__":
    from robot_arm_ik_vive_outpatient import G1_29_ArmIK
    ...
    left_buf = manager.Value("c", None)
    right_buf = manager.Value("c", None)
    left_hand_joint_position = manager.list([0.0] * 6)
    right_hand_joint_position = manager.list([0.0] * 6)

    process_l = multiprocessing.Process(
    ...
    camera_position = np.array([0.1, 0.0, 0.5])
    camera_lookat = np.array([0.7, 0.0, 0.0])

    # left
    left_vis_position = np.array([1.0, 0.7, 0.2])
    offset = (
        (camera_position - camera_lookat)
        / np.linalg.norm(camera_position - camera_lookat)
        * 5.0
    )
    left_vis_transform = compute_billboard_transform(
        camera_position + offset, left_vis_position, np.array([0, 0, 1])
    )
    arm_ik.vis.viewer["left_vis"].set_transform(left_vis_transform)

    # right
    right_vis_position = np.array([1.0, -0.7, 0.2])
    offset = (
        (camera_position - camera_lookat)
        / np.linalg.norm(camera_position - camera_lookat)
        * 5.0
    )
    right_vis_transform = compute_billboard_transform(
        camera_position + offset, right_vis_position, np.array([0, 0, 1])
    )
    arm_ik.vis.viewer["right_vis"].set_transform(right_vis_transform)

    # initial positon
    L_tf_target = pin.SE3(
        pin.Quaternion(1, 0, 0, 0),
        np.array([0.25, +0.2, 0.12]),
    )
    ...
    zero_pose_l = np.array([0.25, 0.2, 0.12])
    zero_pose_r = np.array([0.25, -0.2, 0.12])
    ...
    while True:
        try:
            ...
            if step % 10 == 0:
                arm_ik.vis.viewer[
                    "/Scene/Cameras/default/rotated/<object>"
                ].set_cam_pos(camera_position)
                arm_ik.vis.viewer[
                    "/Scene/Cameras/default/rotated/<object>"
                ].set_cam_target(camera_lookat)
                arm_ik.vis.viewer["/Scene/Lights/SpotLight/<object>"].set_property(
                    "intensity", 0.0
                )
                arm_ik.vis.viewer[
                    "/Scene/Lights/PointLightNegativeX/<object>"
                ].set_property("intensity", 0.0)
                arm_ik.vis.viewer[
                    "/Scene/Lights/PointLightPositiveX/<object>"
                ].set_property("intensity", 0.0)
            # Read device input
        ...
        if left_buf.value is not None:
            material = mg.MeshPhongMaterial(
                map=mg.ImageTexture(mg.PngImage(left_buf.value.getvalue())),
                reflectivity=0.0,
            )
            arm_ik.vis.viewer["left_vis"].set_object(
                mg.Plane(width=0.4, height=0.5), material=material
            )
        if right_buf.value is not None:
            material = mg.MeshPhongMaterial(
                map=mg.ImageTexture(mg.PngImage(right_buf.value.getvalue())),
                reflectivity=0.0,
            )
            arm_ik.vis.viewer["right_vis"].set_object(
                mg.Plane(width=0.4, height=0.5), material=material
            )

        left_hand_array = np.array(left_hand_joint_position)
        right_hand_array = np.array(right_hand_joint_position)
        finger_positions = np.concatenate((left_hand_array, right_hand_array))
